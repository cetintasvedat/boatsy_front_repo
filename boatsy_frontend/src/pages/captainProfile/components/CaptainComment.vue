<template>
  <div class="boat-comment-container" v-if="getCaptainComment.length > 0">
    <div class="user-boat-comment">
      <div class="comment-content" v-for="item in getCaptainComment">
          <div class="user-info">
            <div class="rev-thumb">
              <img
            ...
            <div class="rev-info">
              <div class="user-name">{{uppercaseLetter(item.name)}} {{uppercaseLetter(item.surname)}}</div>
  ...
  <div v-else>
    <div v-if="getProfileModel.userGroupId == 2 ">
      <h3>{{$t('captain_comment')}}</h3>
      <span>{{$t('captain_comment2')}}</span>
    </div>
  </div>
</template>
...
import { FETCH_CAPTAIN_COMMENT } from "../../../store/actions.type.js";
export default {
  data() {
    return {
  ...
  computed: {
    ...mapGetters(["getCaptainComment", "getProfileModel"]),
  },
  async created() {
    this.$store.commit(
      "setCaptainId",
    ...
    this.$store.dispatch(FETCH_CAPTAIN_COMMENT);
  },
  methods: {
    getInsertDate(date) {
